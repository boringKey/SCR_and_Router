...
def format_matrix(mat: List[List[float]], decimals: int = 3) -> str:
    """Return a pretty string representation of a matrix."""
    if not mat:
        return "[]"
    fmt = f"{{:.{decimals}f}}"
    lines = []
...
def compute_scores_with_matrix(
    ...
    """Compute similarity-weighted scores per task using raw similarity as weights."""
...
def compute_scores_grouped(
    ...
    """Compute grouped scores (low/mid/high similarity buckets)."""
    if not zs_mat or not sim_mat:
        return {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compute SCR metrics from metric.json and OT-sim similarity JSON.",
    )
    parser.add_argument("--metric-path", type=str, default="metric.json", help="Path to the metric.json file")
    parser.add_argument("--sim-json", type=str, required=True, help="Path to the similarity JSON generated by OT-sim.py")
    parser.add_argument("--zs-decimals", type=int, default=1, help="Decimals when printing ZS matrix")
    ...
    if not zs_mat:
        ...
        return
    if not sim_mat:
        print("[ERROR] No similarity matrix could be extracted from the OT-sim JSON.")
        return
    ...
    if any(len(row) != sim_cols for row in sim_mat):
        print("[ERROR] similarity_matrix rows in the OT-sim JSON must all have the same length.")
        return
